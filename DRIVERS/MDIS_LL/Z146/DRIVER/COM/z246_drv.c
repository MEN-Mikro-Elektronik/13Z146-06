...
#define Z246_MAX_BUFF_SIZE			4096
#define Z246_RING_SIZE_DEFAULT		0xFFFFFFFF
/*-----------------------------------------+
|  TYPEDEFS                                |
+-----------------------------------------*/
/** low-level handle */
...
)
{
	...
	if((size != 0) && (buf != NULL)){
		if(llDataLen <= Z246_MAX_BUFF_SIZE){
...
u_int32 ReadFromBuffer( LL_HANDLE *llHdl, int8 * result){
	...
	if ((llHdl->ringHead == llHdl->ringTail) || (llHdl->ringDataCnt == 0)) {
		...
		*result = -1;
		...
		llHdl->ringTail = (unsigned int)(llHdl->ringTail + 1) % Z246_MAX_BUFF_SIZE;
		llHdl->ringDataCnt--;
		if (llHdl->ringDataCnt == 0) {
			llHdl->ringTail = Z246_RING_SIZE_DEFAULT;
			llHdl->ringHead = Z246_RING_SIZE_DEFAULT;
...
int8 StoreInBuffer( LL_HANDLE *llHdl , u_int32 data){
	...
	int8 result = 0;
	unsigned int next = (unsigned int)(llHdl->ringHead + 1) % Z246_MAX_BUFF_SIZE;
